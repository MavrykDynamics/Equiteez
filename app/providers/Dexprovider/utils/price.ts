...
export const getPMMTokenPrice = (storage: DodoStorageType) => {
  ...
  const B = new BigNumber(storage.baseBalance).div(new BigNumber(10).pow(3));
  ...
  const Q = new BigNumber(storage.quoteBalance).div(new BigNumber(10).pow(6));
