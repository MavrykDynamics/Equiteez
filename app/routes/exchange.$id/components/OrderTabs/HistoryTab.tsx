    ...
    <div className="flex flex-col w-full gap-4">          
      ...
      <div className="flex w-full gap-4 ">
        ...
        <div className="flex gap-3">
          ...
          <div className={`w-full flex px-3 py-2.5 justify-between eq-input max-w-40 gap-3`}>
            <input 
              name="amount" 
              type="number" 
        ...
        <div className="flex gap-2">
          ...
          <Button  className="opacity-50 pointer-events-none"  variant="outline" size="outline">
            Reset
          </Button>
        </div>
      </div>
              ...
              <table className="min-w-full divide-y divide-gray-300">
                    ...
                    {columns.map((column) => (
                      ...
                      >
                        <span className="flex items-center gap-1">
                          {column.label}
                          { column.sortable ? <ArrowDown className="size-4 cursor-pointer" /> : '' }
                        </span>
                      </th>
                    ))}
                  </tr>
                ...
                <tbody className="divide-y divide-transparent bg-white">
                  {orders.map((order, index) => (
                    <tr key={`${order.date}${index}`} className={`${ order.all == "Cancelled" ? 'opacity-50 pointer-events-none' : ''}`}>


                      <td className="eq-table-cell cursor-pointer"><Plus /></td>
                      ...
                      <td className={`eq-table-cell ${ order.side != "Sell" ? 'text-success' : 'text-error' }`}>
                        {order.side}
                      </td>
