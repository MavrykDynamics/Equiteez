...
export function OpenOrdersTableRow({
  ...
  return (
    <div className={styles.row} role="row">
      ...
      <div className={styles.cell} role="cell">
        <RText size="body-sm">
          <Money tooltip={false}>{order.quote_token.price_per_token}</Money> USDT
        </RText>
      </div>
      <div className={styles.cell} role="cell">
        <RText size="body-sm">
      ...
      <div className={`${styles.cell} ${styles.actionCell}`} role="cell">
        <span aria-label="Cancel order" className={styles.actionIcon} role="img">
          <RIcon name={order.can_cancel ? "trash" : "refund"} size="medium" />
        </span>
      </div>
    </div>
  );
