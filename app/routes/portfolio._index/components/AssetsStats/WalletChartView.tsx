...
export function WalletChartView({
  assets,
  portfolioTotal,
  ...
  const orderedAssets = [...assets]
    .filter(
      (asset) =>
        Number.isFinite(asset.share_pct) && Number.isFinite(asset.balance)
    )
  ...
  const donutChartAssets: AssetsDonutChartAsset[] = chartAssets.map(
    (asset) => ({
