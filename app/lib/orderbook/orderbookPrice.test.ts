...
describe("one-sided depth", () => {
  it("does not use opposite-side liquidity for placement", () => {
    expect(
      getBestLimitAskFromOrderbookDepth({ bids: [{ price: 4 }] }, 6)
    ).toBeNull();
    expect(
      getBestLimitBidFromOrderbookDepth({ asks: [{ price: 5 }] }, 6)
    ).toBeNull();
  });
});
