import { isKnownChainId } from 'app/lib/apis/tzkt/api';
import {
  useDidMount,
  useDidUpdate,
  useSafeState,
  useStopper,
} from 'app/lib/ui/hooks';
...
export default function useHistory(
  ...
  const [loading, setLoading] = useSafeState<TLoading>(
    isKnownChainId(chainId) && 'init'
  );
  ...
  async function loadUserHistory(
    ...
    try {
      ...
      setLoading(false);
      console.error(
        'Logging error in History Hook after fetching history items:',
        error
      );
      return;
    }
