...
import {
  ...
  ParameterFa2,
} from 'app/lib/apis/tzkt/utils';
import { isTruthy } from 'app/lib/utils';

import { MAV_TOKEN_SLUG, toTokenSlug } from '../../assets';
import { AssetMetadataBase } from '../../metadata/types';
...
export function operationsGroupToHistoryItem(
  { hash, operations }: OperationsGroup,
  address: string
): UserHistoryItem {
  ...
  if (operations[operations.length - 1]) {
    oldestOperation = reduceOneTzktOperation(
  ...
  const type = deriveHistoryItemType(
...
function reduceTzktOperations(
  operations: TzktOperation[],
  address: string
): IndividualHistoryItem[] {
  const reducedOperations = operations
    .map((op, index) => reduceOneTzktOperation(op, index, address))
    .filter(isTruthy);
...
function reduceOneTzktOperation(
  ...
  switch (operation.type) {
    ...
    case 'delegation': {
      ...
      const delegationOpBase = buildHistoryItemOpBase(
        operation,
        address,
        0,
        operation.sender,
      ...
      if (operation.newDelegate)
        delegationOp.newDelegate = operation.newDelegate;
      if (operation.newDelegate)
        delegationOp.prevDelegate = operation.prevDelegate;
      return delegationOp;
    }
    case 'origination': {
      const source = operation.sender;
      const contractBalance = operation.contractBalance
        ? operation.contractBalance.toString()
        : '0';
      const originationOpBase = buildHistoryItemOpBase(
        operation,
        address,
      ...
      if (operation.originatedContract)
        originationOp.originatedContract = operation.originatedContract;
      return originationOp;
    }
    case 'reveal': {
      const revealOpBase = buildHistoryItemOpBase(
        operation,
        address,
        0,
        operation.sender,
    ...
    default:
      const source = operation.sender;
      const otherOpBase = buildHistoryItemOpBase(
        operation,
        address,
        0,
        source,
...
function reduceOneTzktTransactionOperation(
  ...
  function _buildReturn(args: {
    ...
    const HistoryOpBase = buildHistoryItemOpBase(
      operation,
      address,
      Number(amount),
      source,
    ...
    if (tokenTransfers) {
      ...
      if (
        ...
        !historyTxOp.entrypoint?.toLowerCase()?.includes('swap')
      ) {
  ...
  if (parameter == null) {
    ...
    const tokenTransfers = buildTokenTransferItem(
      operation,
      MAV_TOKEN_SLUG,
      address
    );
    ...
    const source =
      tokenTransfers?.sender.address === address
    ...
    else if (parameter.value.to === address)
      source.address = parameter.value.from;
    else return null;
...
function buildHistoryItemOpBase(
  ...
  if (!isZero(reducedOperation.amountSigned))
    reducedOperation.amountDiff = getMoneyDiff(reducedOperation.amountSigned);
 ...
 */
function reduceParameterFa2Values(
  values: ParameterFa2['value'],
  relAddress: string
): Fa2TransferSummaryArray {
      ...
      if (val.from_ === relAddress) {
        ...
        if (isValRel && !amount.isZero()) {
          result.push({
            ...
            recipients: [
              {
                to: transformToHistoryMember(relAddress),
                amount: amount.toFixed().toString(),
              },
            ],
          });
...
function stringToHistoryItemStatus(status: string): HistoryItemStatus {
  if (['applied', 'backtracked', 'skipped', 'failed'].includes(status))
    return status as HistoryItemStatus;

  return 'pending';
}

function deriveHistoryItemStatus(
  items: { status: HistoryItemStatus }[]
): HistoryItemStatus {
...
function deriveHistoryItemType(
  ...
  if (firstOperation.type === 'delegation') {
    ...
    if (
      items.some((item) => item?.entrypoint?.toLocaleLowerCase() === 'swap')
    ) {
      ...
      if (
        items.some(
          (item) =>
            item.entrypoint === 'placeSellOrder' ||
            item.entrypoint === 'placeBuyOrder'
        )
      ) {
      ...
      if ('tokenTransfers' in item && item.tokenTransfers)
        if (
          item.tokenTransfers?.recipients?.find((o) => o.to.address === address)
        ) {
...
function transformToHistoryMember(
  address: string,
  alias: string = ''
): TzktAlias {
...
function getDestinationAddress(operation: TzktTransactionOperation) {
  ...
  return diff &&
    !isZero(new BigNumber(diff.content.value)) &&
    typeof diff.content.key === 'string'
