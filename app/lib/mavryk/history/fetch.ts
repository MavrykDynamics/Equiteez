import type { TzktApiChainId, TzktOperation } from 'app/lib/apis/tzkt';
import * as TZKT from 'app/lib/apis/tzkt';
import { MAV_TOKEN_SLUG } from 'app/lib/assets';
import { detectTokenStandard } from 'app/lib/assets/standarts';
import { filterUnique } from 'app/lib/utils';
...
export default async function fetchUserHistory(
  ...
  const arr = groups.map((group) =>
    operationsGroupToHistoryItem(group, accountAddress)
  );
  return arr;
}
...
async function fetchOperations_Any(
  ...
  let newerThen: string | undefined =
    accOperations[accOperations.length - 1]?.timestamp;

  const fa12OperationsTransactions = await TZKT.refetchOnce429(
    () =>
      fetchIncomingOperTransactions_Fa_1_2(
        chainId,
        accountAddress,
  ...
  const fa2OperationsTransactions = await TZKT.refetchOnce429(
    () =>
      fetchIncomingOperTransactions_Fa_2(
        chainId,
        accountAddress,
        newerThen ? { newerThen } : { limit },
        olderThan
...
async function fetchOperGroupsForOperations(
  ...
  for (const hash of uniqueHashes) {
    const operations = await TZKT.refetchOnce429(
      () => TZKT.fetchGetOperationsByHash(chainId, hash),
      1000
    );
