...
import type { UserHistoryItem } from 'app/lib/mavryk/history';
...
// export function fillUserHistoryItemsWithTokenMetadata(
...
export function buildHistoryMoneyDiffs(
  ...
  for (const oper of historyItem.operations) {
    ...
    const assetSlug =
      // @ts-expect-error
      oper.contractAddress == null
        ? MAV_TOKEN_SLUG
        : toTokenSlug(oper.contractAddress, oper.tokenTransfers?.tokenId);
    const diff = new BigNumber(
      oper.type === HistoryItemOpTypeEnum.Origination ? 0 : oper.amountSigned
    ).toFixed();
...
export const getMoneyDiff = (amountSigned: string): string =>
  new BigNumber(amountSigned).toFixed();
