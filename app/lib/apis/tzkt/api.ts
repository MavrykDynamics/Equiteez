...
const TZKT_API_BASE_URLS = {
  ...
  [MavrykChainId.Basenet]: "https://basenet.api.mavryk.network/v1",
...
export const fetchGetOperationsTransactions = (
  chainId: TzktApiChainId,
  params: GetOperationsTransactionsParams
) => fetchGet<TzktOperation[]>(chainId, `/operations/transactions`, params);

export const getOneUserContracts = (
  chainId: TzktApiChainId,
  accountAddress: string
) =>
  fetchGet<TzktRelatedContract[]>(
    chainId,
...
export function fetchTzktAccountAssets(
  ...
  const recurse = async (
    accum: TzktAccountAsset[],
    offset: number
  ): Promise<TzktAccountAsset[]> => {
    const data = await fetchTzktAccountAssetsPage(
      account,
      chainId,
      offset,
      fungible
...
export const fetchGasTokenBalance = (account: string, chainId: string) => {
  if (!isKnownChainId(chainId)) return Promise.resolve(0);

  return fetchGet<TzktRelatedContract[]>(
    chainId,
    `/accounts/${account}/balance`
  );
};

export async function refetchOnce429<R>(
  fetcher: () => Promise<R>,
  delayAroundInMS = 1000
) {
  try {
...
export const fetchTezosBalanceFromTzkt = async (
  account: string,
  chainId: TzktApiChainId
) =>
  getAccountStatsFromTzkt(account, chainId).then(
    calcTzktAccountSpendableTezBalance
  );

export const fetchAllAssetsBalancesFromTzkt = async (
  account: string,
  chainId: TzktApiChainId
) => {
  ...
  await (async function recourse(offset: number) {
    const data = await fetchAssetsBalancesFromTzktOnce(
      account,
      chainId,
      offset
    );
...
const fetchAssetsBalancesFromTzktOnce = (
  account: string,
  chainId: TzktApiChainId,
  offset = 0
) =>
