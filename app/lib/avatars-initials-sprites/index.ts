...
export default function initialsSprites(random: Random, options: Options = {}) {
  ...
  if (options.background) {
    ...
    Object.keys(Color.collection).forEach((backgroundColorInner) => {
      const bgColorInner = backgroundColorInner as keyof ColorCollection;

      if (
        options.backgroundColors === undefined ||
        options.backgroundColors.length === 0 ||
        options.backgroundColors.indexOf(bgColorInner) !== -1
      ) {
        backgroundColors.push(
          Color.collection[bgColorInner][options.backgroundColorLevel!]
        );
      }
    });
  }
